...
- Новости за 24ч: ИИ, крипта, геополитика
...
```
fetch_news_body()
  → 3× OpenRouter (perplexity/sonar, parallel, 30s timeout)
  → модель: SUMMARY (RU) + LINK (url | title), до 4 ссылок
  → parse: annotations/citations whitelist, format plain block
  → report._format_news_body() → Telegram HTML
```

Полный дайджест: `build_full_digest_html()` — погода + курсы + новости + мотивация, **без Gemini**.
